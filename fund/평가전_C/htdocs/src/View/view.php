...
<div id="view-fund" class="py-5">
    <div class="container">
        <div class="header flex-col-center">
            <i class="fa fa-bar-chart pb-1"></i>
            <h2>fund <span>View</span></h2>
            <p>다른 펀드들은 어떨까요?</p>
        </div>
        <div class="row">
            ...
            <div class="col-lg-4 col-md-6 my-3 px-4">
                <div class="card">
                    ...
                    <div class="card-footer flex-center">
                        <?php if($fund->fund_success >= 100 && $fund->owner == user()->user_email) { ?>
                        <form class="w-50 mx-1 py-0 px-0 blue-btn text-center" action="/insert/business" method="post">
                        ...
                        <form action="/detail" method="post" class="w-50 mx-1">
                                <input type="hidden" name="fund_num" value="<?=$fund->fund_num?>">
                                <button class="blue-btn w-100">상세보기</button>
                        </form>
                        <?php } else if($fund->fund_success < 100) { ?>
                        <button id="invest-btn" class="w-50 mx-1 blue-btn" data-toggle="modal" data-target="#invest-modal" data-num="<?=$fund->fund_num?>" data-name="<?=$fund->fund_name?>" data-max="<?=$fund->total?>">투자하기</button>
                        <form action="/detail" method="post" class="w-50 mx-1">
                                <input type="hidden" name="fund_num" value="<?=$fund->fund_num?>">
                                <button class="blue-btn w-100">상세보기</button>
                        </form>
                        <?php } else { ?> 
                        <form action="/detail" method="post" class="w-100 mx-1">
                                <input type="hidden" name="fund_num" value="<?=$fund->fund_num?>">
                                <button class="blue-btn w-100">상세보기</button>
                        </form>
                        <?php } ?>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-body px-4 py-5">
                ...
                <div class="form-group">
                    <button id="close-btn" class="blue-btn w-100" data-bs-dismiss="modal">취소</button>
                </div>
            </div>
        </div>
    </div>
