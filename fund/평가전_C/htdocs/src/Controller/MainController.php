...
class MainController {
    ...
    function insertBusiness() {
        ...
        $fund = DB::fetch("SELECT fund_current, fund_num FROM funds WHERE fund_num = ?", [$fund_num]);
        ...
        DB::query("DELETE FROM investors WHERE fund_num = ?", [$fund->fund_num]);
        
        go("/view", "사업 추가가 완료되었습니다.");
    }

    function registerPage() {
        if(!user()) go("/login", "로그인한 사용자만 이용할 수 있는 페이지입니다.");
    ...
    function insertFund() {
        ...
        DB::query("INSERT INTO funds(fund_num, fund_name, fund_endDate, fund_total, owner) 
                   VALUES(?, ?, ?, ?, ?)", [$fund_num, $fund_name, $fund_endDate, $fund_price, user()->user_email]);
